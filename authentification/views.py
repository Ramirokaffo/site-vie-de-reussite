...
def index(request):        
        if request.POST:
            ...
            # return render(request, "auth/ask_email.html", context={"title": "Connexion"})
...
def register(request):
    ...
    if request.POST:
        ...
        sendActivationEmail(request=request, confirm_subject=confirm_subject, myUser=myUser)
...
def activate(request: WSGIRequest, uidb64, token):
    if not request.POST:
        ...
        sendActivationEmail(request=request, confirm_subject=confirm_subject, myUser=expexted_user)

    

def authMail(request: WSGIRequest):
    if not request.POST:
        return render(request, "auth/ask_email.html", context={"title": "MAail de recupération/activation"})
    else:
        username = request.POST.get("username")
        try:
            expected_user = User.objects.get(username=username)
            confirm_subject = "Changement de mot de passe"
            sendActivationEmail(request=request, confirm_subject=confirm_subject, myUser=expected_user)            
...
def sendActivationEmail(request, myUser, confirm_subject):
